...
if ($(yc config profile get otus-kuber-stage)) {

   # Activate YC CLI profile for sa-otus-kuber-stage-tf service account (if exists)
   yc config profile activate otus-kuber-stage
   ...
   # Create new YC CLI profile for sa-otus-kuber-stage-tf service account
   ...
   $folder               = $(yc resource-manager folder get stage-folder --format json) | ConvertFrom-Json
   $Env:TF_VAR_folder_id = $folder.id

   yc iam key create --service-account-name sa-otus-kuber-stage-tf --folder-name stage-folder --output .\secrets\key.json
   yc config profile create otus-kuber-stage
